const fs = require('fs');
let code = fs.readFileSync('app/smart-planner/page.tsx', 'utf8');

// The file is huge, let's write a simplified version.
let simplified = `'use client';

import React, { useState, useRef, useEffect, useCallback } from 'react';
import { useRouter } from 'next/navigation';
import { ArrowLeft, Loader2, Download, RotateCcw, ImagePlus, X, RefreshCw, Undo2, Redo2, Maximize2, Move } from 'lucide-react';
        ...
        <header className="flex-none flex items-center justify-between p-4 bg-[#0a0a0a] border-b border-[#1f1f1f]">
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              <h1 className="text-xl font-bold tracking-widest text-zinc-100 uppercase flex items-center gap-3">
                ...
                <span className="px-2 py-0.5 text-[10px] bg-[#00f0ff]/20 text-[#00f0ff] rounded font-mono">BETA</span>
              </h1>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center space-x-4 bg-black px-4 py-2 rounded border border-zinc-800 font-mono text-sm shadow-inner">
              <span className="text-zinc-500">Trace: <span className="text-[#00f0ff] font-bold">{currentAreaSqm.toFixed(1)} sqm</span></span>
            </div>
...
fs.writeFileSync('app/concept-generator/page.tsx', simplified);
