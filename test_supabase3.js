...
async function test() {
  ...
  const { data, error } = await supabase
    .from('projects')
    .select('session_id, updated_at, projectName:state->>projectName, placeName:state->>placeName, isDeleted:state->>isDeleted, finalRenderState:state->>finalRender, currentFloorPlanState:state->>currentFloorPlan, project_images(current_floor_plan, final_render)')
